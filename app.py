...
def start_server():
    ...
    print("=" * 72)
    print("  ROBOTIC AI AGENT FOR INVENTORY MONITORING (TAVILY-POWERED)")
    print("=" * 72)
    print(f"  [+] Status: Server successfully running on localhost")
    print(f"  [+] URL:    {url}")
    print(f"  [+] Mode:   {'Live Tavily API' if SYSTEM_STATE['tavily_api_key'] else 'High-Fidelity Simulation Mode (No API key needed!)'}")
    print("=" * 72)
    print("  [>] Press Ctrl+C in this terminal window to stop the server.")
    print("=" * 72)

    # Automatically launch user's web browser after half a second
    def open_browser():
        time.sleep(0.8)
        webbrowser.open(url)
